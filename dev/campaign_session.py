...
def main():
    ...
    os.environ.update(
        ...
        DBUS_SYSTEM_BUS_ADDRESS="unix:path=" + os.environ["XDG_RUNTIME_DIR"] + "/disabled-system-bus",
    )
    ...
    saved_run = Path(os.environ["CAMPAIGN_SAVED_RUN"]) if os.environ.get("CAMPAIGN_SAVED_RUN") else None
    ...
    class CampaignApplication(MluvaApplication):
        ...
        def _live_rewrite_finished(self, session_id, client, revision, result, model):
            ...
            value = super()._live_rewrite_finished(session_id, client, revision, result, model)
    ...
    def observed_connect(*args, **kwargs):
        ...
        class ObservedConnection:
            ...
            def send(self, message):
                ...
                self.sent_bytes += len(base64.b64decode(payload.get("audio_base_64", "")))
                if payload.get("commit"):
                    event("outbound_commit", audio_offset_seconds=self.sent_bytes / 32000)
    ...
    def exercise():
        ...
        try:
            ...
            ipc("layout")
            assert xlib.XGetSelectionOwner(display, xlib.XInternAtom(display, b"_NET_WM_CM_S0", 0))
            screenshot("ready")
            if scenario.startswith("layout"):
                ipc("stageImage", str(output / "stage.png"))
                settle(lambda: json.loads(ipc("state"))["stageSaved"])
            if scenario == "catalog":
            ...
            if scenario.startswith("saved"):
                ...
                screenshot("hero")
                (output / "recognition.json").write_text(json.dumps(asdict(entry), indent=2) + "\n")
                (output / "replies.json").write_text(json.dumps([asdict(reply) for reply in replies], indent=2) + "\n")
                event(
                    "saved_render",
            ...
            if app.pending_realtime_fallback_reason:
                raise RuntimeError("Realtime provider did not open; no batch fallback recording is accepted")
            playback = launch(
                [
                    "pw-play",
                    "--target=0",
            ...
            def audio_nodes_ready():
                ...
                audio_nodes[:] = [
                    ...
                    and node["info"]["props"]["node.name"] in {"campaign-playback", "pw-record"}
                ]
                return len(audio_nodes) == 2

            settle(audio_nodes_ready)
            for node in audio_nodes:
                direction = "Input" if node["info"]["props"]["node.name"] == "pw-record" else "Output"
                configuration = (
                    "{ direction = " + direction + " mode = dsp format = { mediaType = audio mediaSubtype = raw "
            ...
            def ports_ready():
                ports[:] = subprocess.check_output(["pw-link", "-o"], text=True).splitlines()
                inputs = subprocess.check_output(["pw-link", "-i"], text=True).splitlines()
                return any("campaign-playback:" in port for port in ports) and any(
                    "pw-record:" in port for port in inputs
                )

            settle(ports_ready)
            output_port = next(port.strip() for port in ports if "campaign-playback:" in port)
            input_port = next(
                port.strip()
                for port in subprocess.check_output(["pw-link", "-i"], text=True).splitlines()
            ...
            if scenario == "task":
                event(
                    "task_outcome",
                    missing_fields_visible=bool(replies and "[Missing:" in replies[-1].text),
                    final_update_complete=bool(replies and "partial draft" not in replies[-1].instruction),
    ...
    try:
        ...
        settle(lambda: (Path(os.environ["XDG_RUNTIME_DIR"]) / "pipewire-campaign").exists())
        shell = launch(
        ...
        if recorder is not None and recorder.poll() is None:
            try:
                ...
                errors.append(f"Recorder shutdown: {type(error).__name__}")
        ...
        for child in children:
            if child.poll() is None:
                try:
                    ...
                    leaked = bool(set(credentials) & keys)
                    service_environments.append({"pid": child.pid, "provider_credentials_present": leaked})
                    if leaked:
                        errors.append("Provider credential found in a private service environment")
                except FileNotFoundError:
                    pass  # The child exited between poll and /proc inspection.
                except OSError as error:
                    errors.append(f"Service audit: {type(error).__name__}")
...
if __name__ == "__main__":
    try:
        ...
        # Also clean up if setup failed before the service/receipt try/finally.
        provider_home = Path(os.environ["OFFSCREEN_SESSION_ROOT"]) / "codex-provider"
