...
def main() -> None:
    ...
    font = Path(run(["fc-match", "-f", "%{file}", "Adwaita Sans:style=Semibold"]).strip())
    ...
    for index, clip in enumerate(clips):
        ...
        filters.append(f"[v]trim=duration={seconds},setsar=1,fade=t=in:d=0.1:color=0x202630,format=yuv420p[vout]")
    ...
    receipt = {
        ...
        "media": output_media,
        "transitions": "Short 100 ms edge reveals, continuous eased camera motion",
        "audio": None
        if args.preview
