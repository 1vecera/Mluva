...
The earlier combined provider/Live branch passed all 358 tests with `make linux-test`, feature maturity validation, Ruff lint and formatting. `make linux-shortcut-test` passed the private portal lifecycle. `make linux-text-target-test linux-conversation-test linux-live-rewrite-test` passed on reserved `:192`, covering real cross-process AT-SPI focus capture, restoration and exact Unicode insertion, all 14 conversation scenarios and the Live workspace. This included provisional input kept out of raw text, mandatory final reconciliation for matching previews, manual edits during an in-flight final update, one saved final reply, final-only rewrite copying, failed-final labeling, cancellation and Incognito. The subsequent [delight launch verification](verification/delight-launch.md) records the current runtime, 381-test suite and additional native checks.

The GTK motion fixture samples actual scroll positions, including repeated updates with the same destination and full draft replacement while reading above the bottom. The current QML fixture samples intermediate scroll and recording-light size, keeps five preview lines and the timer, checks bounded Unicode prefix changes, and verifies static output when smooth scrolling is off or duration is zero. The September delight update replaces the visible status word and earlier opacity animation with a 2.6-second radial pulse: its six-pixel center grows and shrinks 18% around its original size inside a fixed slot, with no sideways motion. The native GTK controls use the same pulse, honor the desktop animation setting, and stop their frame callbacks when hidden. QML follows the configured smooth-scrolling preference; its [SmoothedAnimation](https://doc.qt.io/qt-6/qml-qtquick-smoothedanimation.html) eases scrolling across the full configured duration.
...
- `tests/recording_control_smoke.py` for the native recording light, fixed allocations, theme contrast, reduced motion and callback cleanup on unmap.
...
This timing pass did not perform a live desktop installation or physical Wayland shortcut acceptance. It also did not run Swift/macOS build and launch checks or GNOME Shell runtime checks. The provider/Live work has since been merged; Experimental acceptance limits remain in the [feature-maturity register](feature-maturity.md).
