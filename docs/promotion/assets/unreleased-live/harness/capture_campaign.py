...
def main() -> None:
    ...
    commit = subprocess.check_output(
        ["git", "rev-parse", "v0.3.0^{commit}"], cwd=root, text=True
    ).strip()
    ...
    runtime_diff = subprocess.check_output(
        ["git", "diff", runtime_commit, "--", "linux"], cwd=runtime
    )
    ...
    (output / "runtime-files.json").write_text(
        json.dumps(runtime_hashes, indent=2) + "\n"
    )
    ...
    credentials = {}
    if args.scenario in {"speech", "task"} and args.stt == "elevenlabs":
        credentials["ELEVENLABS_API_KEY"] = os.environ[
            "DAS_ITEM_ELEVEN_LABS_API_KEY__CREDENTIAL"
        ]
    if args.scenario == "task" and args.rewrite_provider == "litellm":
        credentials["CAMPAIGN_REWRITE_KEY"] = os.environ[
            "DAS_ITEM_ZAI_API_KEY__CREDENTIAL"
        ]
    ...
    def serve() -> None:
        ...
        try:
            with server.accept()[0] as connection:
                ...
                if (
                    uid != os.getuid()
                    or str(root / "dev/campaign_session.py").encode() not in command
                ):
    ...
    environment = {
        ...
        + (hashlib.sha256(runtime_diff).hexdigest()[:12] if runtime_diff else runtime_commit[:12]),
        ...
        "CAMPAIGN_REWRITE_MODEL": args.rewrite_model
        or ("glm-4.7-flash" if args.rewrite_provider == "litellm" else ""),
        "CAMPAIGN_CODEX_BINARY": shutil.which("codex") or "",
    }
    helper = (
        Path.home()
        / ".agents/skills/run-offscreen-linux-verification-daniel/scripts/run_isolated_x11.sh"
    )
    command = [
        "bash",
        str(helper),
    ...
    if any(
        hashlib.sha256((runtime / name).read_bytes()).hexdigest() != digest
        for name, digest in runtime_hashes.items()
    ):
        raise RuntimeError("Runtime files changed during capture")
    (output / "runtime-verification.json").write_text(json.dumps({
        ...
        "runtime_files_manifest_sha256": hashlib.sha256((output / "runtime-files.json").read_bytes()).hexdigest(),
    }, indent=2) + "\n")
    if result or errors:
        raise RuntimeError(
            f"Capture failed; inspect {output}/session.log; broker={errors}"
        )
