...
Custom model choices support search. **Enter model ID…** allows an explicit alias when the catalog is unavailable or omits a deployment. A listed model is not proof that the account can use it, and a model absent from a compatible server’s listing may still work. Codex keeps its existing login and model configuration, while Mluva disables execution environments, tools, apps, MCP, hooks and skills. Global instruction files are hidden in a child mount namespace using bubblewrap; the original files and login store stay intact. An older Codex that cannot confirm isolation is refused; the protocol is verified with Codex 0.155.1. Local models use a size slider; see [model choices](local-speech-models.md).
